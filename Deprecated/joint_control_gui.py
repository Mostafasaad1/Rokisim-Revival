...
class Worker(QObject):
    ...
    def __init__(
        self, sender: rokisim_sender.RoKiSimSender, num_joints: int, max_speeds: list
    ):
        super().__init__()
        self.sender = sender
            ...
            if has_significant_change and self.num_joints > 0:
                try:
                    ...
                    self.update_signal.emit(
                        self.joint_angles
                    )  # Emit signal to update sliders
                    logging.debug(
                        f"Sent angles: {[round(a, 2) for a in self.joint_angles]}"
                    )
...
class JointControlGUI(QWidget):
    ...
    def init_ui(self):
        ...
        self.no_robot_label = QLabel(
            "No robot loaded. Please load an XML file to begin."
        )
    ...
    def _reload_robot_definition(self):
        ...
        # Update window title
        self.setWindowTitle(
            f"RoKiSim Joint Control - {self.robot_definition.name} ({self.NUM_JOINTS} Joints)"
        )
        ...
        if self.NUM_JOINTS > 0:
            for i in range(self.NUM_JOINTS):
                ...
                reset_button.clicked.connect(
                    lambda checked, idx=i: self._reset_joint_to_default(idx)
                )
    ...
    def start_worker_thread(self):
        if self.NUM_JOINTS > 0:
            ...
            self.worker.fk_update_signal.connect(
            ...
            if dt > 0.05:  # Update speed display at reasonable intervals (20 FPS)
                for i, angle in enumerate(angles):
                    if i < len(self.sliders) and i < len(self.joint_spinboxes):
                        ...
                        if hasattr(self, "joint_speed_gauges") and i < len(
                            self.joint_speed_gauges
                        ):
    ...
    def _on_calculate_ik(self):
        ...
        try:
            ...
            ik_solution = self.sender.calculate_ik(
                target_pose, initial_guess=self.worker.joint_angles
            )
    ...
    def compile_program(self):
        ...
        try:
            ...
            self.program_status_label.setText(
                f"Program compiled successfully. {len(self.compiled_program)} positions generated."
            )
    ...
    def execute_program(self):
        ...
        try:
            ...
            for i, target_position in enumerate(self.compiled_program):
                ...
                self.program_status_label.setText(
                    f"Executing step {i+1}/{len(self.compiled_program)}"
                )
                ...
                while not movement_complete and (time.time() - start_time) < 5.0:
                    ...
                    for j in range(len(target_position)):
                        if j < len(self.worker.joint_angles):
                            error = abs(
                                self.worker.joint_angles[j] - target_position[j]
                            )
