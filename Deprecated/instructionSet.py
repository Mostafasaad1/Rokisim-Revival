...
class EnhancedRobotCompiler:
    def __init__(self, interpolation_steps=10):
        ...
        self.default_interpolation_steps = (
            interpolation_steps  # Default steps for interpolation
        )
        ...
        while pc < len(lines):
            ...
            if command == "SETE":
                ...
                if self.current_pose is None:
                    raise ValueError(
                        "MOVL requires a current position. Add a MOVJ command first."
                    )
                ...
                if self.current_pose is None:
                    raise ValueError(
                        "MOVC requires a current position. Add a MOVJ command first."
                    )
    ...
    def _evaluate_expression(self, expr):
        ...
        if "+" in expr:
            parts = expr.split("+")
            return self._evaluate_expression(parts[0]) + self._evaluate_expression(
                parts[1]
            )
        elif "-" in expr:
            parts = expr.split("-")
