...
class RoKiSimSender:
    ...
    def load_robot_definition(self, xml_file_path):
        ...
        try:
                ...
                if axis_id:
                    try:
                        ...
                        if limsup_str is not None and liminf_str is not None:
                            ...
                            has_limits = True
                            logging.debug(f"  Using limsup/liminf: [{min_limit}, {max_limit}]")
                        elif maxvalue_str is not None and minvalue_str is not None:
                            max_limit = float(maxvalue_str)
                            min_limit = float(minvalue_str)
                            has_limits = True
                            logging.debug(f"  Using maxvalue/minvalue: [{min_limit}, {max_limit}]")
                        ...
                        if axis_id.startswith('Joint') and has_limits:
                            ...
                            logging.debug(f"  Added as active joint {len(self.robot_definition.joint_min_angles)}: {axis_id}")

                    except (ValueError, TypeError, AttributeError) as e:
                        logging.warning(f"Error parsing axis {axis_id}: {e}")

            num_loaded_joints = self.robot_definition.get_num_joints()
            logging.info(f"Loaded DH parameters for {len(self.robot_definition.dh_params)} axes.")
            logging.info(f"Loaded joint limits for {num_loaded_joints} joints.")

            if num_loaded_joints > 0:
                logging.debug(f"Sample Limits for Joint1: Min={self.robot_definition.joint_min_angles[0]}, Max={self.robot_definition.joint_max_angles[0]}")
            ...
            logging.error(f"Failed to parse XML file '{xml_file_path}' (lxml syntax error): {e}")
            raise
        except FileNotFoundError:
            logging.error(f"XML file not found: '{xml_file_path}'")
            raise
        except Exception as e:
            logging.error(f"Unexpected error loading robot definition from '{xml_file_path}': {e}")
            raise
        ...
        if self._pybotics_robot is None:
            try:
                ...
                if len(dh_params) == self.protocol_num_joints:
                    ...
                    logging.error("Cannot create Pybotics Robot object: Incorrect number of DH parameters.")
                    self._pybotics_robot = None
            except Exception as e:
    ...
    def validate_angles(self, angles):
        ...
        if len(angles) != num_loaded_joints:
            logging.error(f"validate_angles: {num_loaded_joints} angles required, got {len(angles)}.")
            return False

        if num_loaded_joints == 0:
            logging.warning("validate_angles: No joint limits loaded. Validation skipped.")
            return True
        ...
        for i in range(num_loaded_joints):
            if not (min_angles[i] <= angles[i] <= max_angles[i]):
                logging.warning(f"Angle {i+1} ({angles[i]}) is out of range "
    ...
    def calculate_fk(self, joint_angles):
        ...
        try:
            ...
            if len(joint_angles) != expected_dofs:
                 raise ValueError(f"FK requires {expected_dofs} joint angles, got {len(joint_angles)}.")

            position, orientation_rpy = kinematics.forward_kinematics(robot, joint_angles)
    ...
    def calculate_ik(self, target_pose, initial_guess=None):
        ...
        try:
            ...
            solution = kinematics.inverse_kinematics(
                robot,
                target_pose,
                initial_guess_deg=initial_guess
            )
            return solution
        except Exception as e:
    ...
    def send_angles(self, angles):
        ...
        if num_to_send != num_loaded_joints:
            raise ValueError(f"Exactly {num_loaded_joints} joint angles are required based on loaded definition.")
        ...
        try:
            ...
            logging.debug(f"Data sent successfully to {self.ip}:{self.port} ({len(angles_to_send)} joints)")
        except (socket.timeout, ConnectionRefusedError, OSError) as e:
            logging.error(f"Network error sending data to RoKiSim ({self.ip}:{self.port}): {e}")
            raise
        except Exception as e:
        ...
        if pos and rpy:
            ...
            if ik_solution:
                ...
                if verify_pos and verify_rpy:
                    print(f"Verification - Position: {verify_pos}, Orientation: {verify_rpy}")
