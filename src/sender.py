...
class RoKiSimSender:
    ...
    def load_robot_definition(self, xml_file_path: str) -> None:
        name, min_angles, max_angles, dh_params, axis_index = self._parser.parse(xml_file_path)
    ...
    def validate_angles(self, angles: List[float]) -> bool:
        ...
        if len(angles) != num_loaded:
            logging.warning(f"Invalid number of angles: {num_loaded} required, got {len(angles)}.")
            return False
        if num_loaded == 0:
            return True
    ...
    def calculate_fk(self, joint_angles: List[float]) -> Tuple[Optional[Tuple[float, float, float]], Optional[Tuple[float, float, float]]]:
    ...
    def calculate_ik(self, target_pose: Tuple[float, ...], initial_guess: Optional[List[float]] = None) -> Optional[List[float]]:
        robot = self._get_or_create_robot()
    ...
    def send_angles(self, angles: List[float]) -> None:
        ...
        index = 8
        angles_to_send = angles[:self.protocol_num_joints] if len(angles) > self.protocol_num_joints else angles
